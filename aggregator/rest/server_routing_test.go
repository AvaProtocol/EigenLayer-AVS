...
func TestColonActionRouting(t *testing.T) {
	...
	routes := []struct {
		...
		{"POST", "/wallets/:address/delegation:prepare", "DELEGATION_PREPARE"},
		{"POST", "/wallets/:address/delegation:submit", "DELEGATION_SUBMIT"},
...
func TestShouldDropRoute(t *testing.T) {
	cases := []struct {
		...
		{"/workflows/:id", false},
		{"/auth:exchange", true},
		{"/wallets/:address/delegation:prepare", true},
		{"/wallets/:address/delegation:submit", true},
		{"/wallets/:address/delegation", false},
