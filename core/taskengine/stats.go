...
// NewStatService constructs a StatService. Task storage is chain-agnostic
// (G5), so counting scans a single per-wallet prefix regardless of chain.
...
func (svc *StatService) GetTaskCount(smartWalletAddress *model.SmartWallet) (*model.SmartWalletTaskStat, error) {
	...
	// Task storage is chain-agnostic (G5): a single "u:{owner}:{wallet}"
	// prefix matches every task for this wallet regardless of chain.
