			...
			if err := json.Unmarshal([]byte(body), &bodyObj); err == nil {
				...
				case "sendgrid":
					...
					if !isSingleNodeImmediate(r.vm) && !hasAISummary {
						...
						// All email content is generated by context-memory, aggregator just passes it through
						if r.vm.logger != nil {
							r.vm.logger.Info("Using context-memory summary as pass-through",
								"subject", summaryForClient.Subject,
								"hasStatusHtml", summaryForClient.StatusHtml != "",
								"hasBranchSummary", summaryForClient.BranchSummaryHtml != "",
								"skippedNodesCount", len(summaryForClient.SkippedNodes))
						}

						// Pass through all fields from context-memory API response
						dynamicData["subject"] = summaryForClient.Subject

						if summaryForClient.AnalysisHtml != "" {
							dynamicData["analysisHtml"] = summaryForClient.AnalysisHtml
						}
						...
						// Pass through branch summary HTML from context-memory (for partial success scenarios)
						if summaryForClient.BranchSummaryHtml != "" {
							dynamicData["branchSummaryHtml"] = summaryForClient.BranchSummaryHtml
						}

						// Pass through skipped nodes list from context-memory
						if len(summaryForClient.SkippedNodes) > 0 {
							dynamicData["skippedNodes"] = summaryForClient.SkippedNodes
						}
