...
func TestTerminalStatesCannotBeToggled(t *testing.T) {
	...
	updates := map[string][]byte{
		string(ChainWorkflowStorageKey(int64(1), taskObj.Id, avsproto.TaskStatus_Completed)): taskJSON,
		string(ChainTaskUserKey(int64(1), taskObj)):                                          []byte(fmt.Sprintf("%d", avsproto.TaskStatus_Completed)),
	}
	err = engine.db.BatchWrite(updates)
	assert.NoError(t, err)
	// Delete old key
	_ = engine.db.Delete(ChainWorkflowStorageKey(int64(1), taskObj.Id, oldStatus))
