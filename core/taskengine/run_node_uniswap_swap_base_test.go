...
func setupUniswapSwapTest(t *testing.T) *uniswapSwapTestSetup {
	...
	t.Logf("📋 Base Uniswap Swap Test Configuration:")
...
func checkUSDCAllowance(t *testing.T, setup *uniswapSwapTestSetup) *big.Int {
	...
	allowance, err := usdcContract.Allowance(nil, *setup.smartWalletAddr, common.HexToAddress(BASE_SWAPROUTER))
...
func TestRunNodeImmediately_UniswapSwap_Base(t *testing.T) {
	...
	usdc, err := erc20.NewErc20(common.HexToAddress(BASE_USDC), setup.client)
	require.NoError(t, err, "Failed to create USDC contract binding")
	...
	router := common.HexToAddress(BASE_SWAPROUTER)
	...
	t.Logf("   Getting quote for %s USDC → WETH...", BASE_SWAP_AMOUNT)
	...
	t.Logf("     amountIn: %s (0.01 USDC)", BASE_SWAP_AMOUNT)
