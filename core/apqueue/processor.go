...
func (p *Processor) Perform(job *Job) error {
	if job == nil {
		return
	}
...
func (c *ContractProcessor) Perform() {
	...
	txResult, err := SendUserOp(conn, bundlerClient, signer.MustControllerSigner(), owner, calldata)
	if err != nil || txResult == "" {
