...
- Seed `tokenwhitelist/arbitrum.json` with WETH / native USDC / USDC.e / USDT / ARB (official addresses). Protocols 0.10.0 does not yet ship an Arb tokens sidecar, so this is a hand-seed, not `make sync-tokens`.
