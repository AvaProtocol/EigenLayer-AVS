...
**ERC-20 amounts (A7-blocking for this card):** Studio cannot ship a multi-token amount list until `erc20SpendCaps[]` is on the gateway. A capped token may only have `transfer`/`approve` on its allowlist — **do not cap WETH on a Uniswap row that also allows deposit/withdraw**. Cap USDC (approve) and, separately, WETH on a Send-ERC-20 (`transfer`) row. Until A7: one `erc20SpendCap` token. After A7: list of amount inputs on **this** grant. Native ETH is **not** this field.
...
5. Studio: amount list on the shared grant. **A token can be capped only if its `allowedActions` are a subset of `transfer`/`approve`.** AllowlistModule reverts `InvalidCalldataLength` / `SelectorNotAllowed` on `deposit`/`withdraw` (and anything else) when `hasERC20SpendLimit` is set — one row per target, cannot split. Uniswap purpose-complete WETH (approve+deposit+withdraw) **cannot** also carry a WETH spend cap. Cap WETH on a Send-ERC-20 row (`transfer` only). Cap USDC on Uniswap (`approve` only). [studio#1676](https://github.com/AvaProtocol/studio/issues/1676) is “WETH transfer/approve uncapped”, not wrap. Expiry copy stays: one `TimeRange` for the shared grant.
