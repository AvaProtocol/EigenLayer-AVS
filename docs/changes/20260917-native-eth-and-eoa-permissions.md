...
PRs target `staging`. Conventional Commits. `make storage-check` before anything that touches persisted models/keys. PR #788 audit (2026-09-17) is folded into K7, K14, A0, A1, A3. Multi-token ERC-20 caps are **A7** (K15): **UI-blocking**, independent of native send, **not** after A3.
...
- Do **not** put native ETH in `erc20SpendCap` / a future `erc20SpendCaps[]`. ETH send is `nativeSpendCap` + `NativeTokenLimitModule`.
- Do **not** add a second “Send ERC-20” catalog row (or a second grant) to cap another token. One usable grant per `(chainId, runner)`; another submit **replaces**.
- Do **not** implement `erc20SpendCaps[]` inside A1 (native fields). A7 is its own PR and **blocks Studio** from adopting the shared-grant amount list. Do **not** wait for A3.
...
| K14 | **Verification-gas seed scales with allowlist rows; max native recipients is 5.** `seedVerificationGasDeferredHooks` (700k) covers ~3 rows. `SessionAuthorization.AllowlistRows` is set from the packed install in `NewSessionResolver` (same site as `DeferredTeardownCount`). Extra rows add `seedVerificationGasPerAllowlistRow` (45k) — a **two-point interpolation** (3 rows ≈ 700k, 20 rows ≈ 1.5M), not a measured per-row cost. A0: 20-row first-op **install** mined at 1.5M VGL (~1.19M actual). 20-row deferred **replace** AA23'd 3/3 at 2.2M (one lucky 1.8M mine). `SubmitSessionPolicy` supersedes on every re-grant, so replace is the normal path. Product max is therefore **5** (`MaxNativeRecipients` in `session_permissions.go`), not 20. `seedVerificationGasPerUninstall` stays 120k (measured UserOp delta on small grants); the 448k figure is owner-tx `eth_estimateGas` of a 20-row 5-hook uninstall, not UserOp VGL, and is moot once the max is 5. | 20-row install passes; 20-row replace does not. Same K14 gate, different operation. A1 must not ship 20. |
...
**ERC-20 amounts + wrap vs WETH cap (singleton):** one grant per `(chainId, runner)`. AllowlistModule can cap a token **only** if that token’s **unioned** selectors are a subset of `transfer`/`approve`. Uniswap wrap adds WETH `deposit`/`withdraw`, so **a WETH spend cap and Uniswap wrap cannot coexist on the same grant**. Gateway `Validate` 400s that shape; Studio must not send it.

**Resolution (wrap wins, K0):** compile the selector union first. Then **omit** any spend cap whose token is not transfer/approve-only. Enabling Send ERC-20 while Uniswap wrap is On still merges (USDC cap stays; WETH `transfer` may be allowlisted **uncapped**). Do **not** drop deposit/withdraw to attach a WETH amount. Do **not** disable the Uniswap toggle. Copy on the WETH amount: “WETH can’t be spend-capped while Uniswap wrap is on (deposit/withdraw). USDC is still capped.” Optional later: an explicit “cap WETH, disable wrap” choice — not the default.

Until A7: one `erc20SpendCap` token. After A7: amount list on this grant, with that omit rule. Native ETH is **not** this field.
...
if nativeSend:
    ...
    max 5 recipients (MaxNativeRecipients; 20-row replace AA23s)
...
**A1 packing:** A2 landed first, so `HooksFor` packs native rows and NT hooks when `nativeSpendCap` is set. ERC-20-only grants stay the 3-hook layout. Native-only still installs `AllowlistExecHook` (K5).
...
→ nativeRecipients = {alice, bob}   // union, dedupe, max 5
...
#### A.7 Per-token ERC-20 caps (`erc20SpendCaps[]`) — **blocks Studio UI** (K15)
...
| Want | Until A7 | A7 |
| --- | --- | --- |
| Cap USDC at 500 | `erc20SpendCap: { token: USDC, amount }` | Same, as a one-element list |
| Also cap WETH | **Not on the wire.** Second Send ERC-20 enable merges purposes and **keeps** the existing cap token | `erc20SpendCaps: [USDC, WETH]` packed as two `HasERC20SpendLimit` rows |
| Send native ETH | Track A `nativeRecipients` + `nativeSpendCap` | Unchanged — not this array |
| Multi-token amounts in UI | One `APInput`; merged/On read-only | List of amount inputs on **one** grant, not another catalog row |

**Doable on-chain without a new module:** same `installValidation`, same Allowlist exec hook, same `executeUserOp` wrap. Extra SSTORE per extra capped token at install. Runtime is O(1) per transfer.
...
5. Studio: amount list on the shared grant. **Cap only tokens whose unioned selectors ⊆ {transfer, approve}.** Uniswap wrap + WETH cap cannot share a grant; **wrap wins** — omit the WETH cap, keep deposit/withdraw, copy on the amount field (§A.0.1). Do not treat catalog rows as separate grants. [studio#1676](https://github.com/AvaProtocol/studio/issues/1676) is uncapped WETH **transfer/approve**, which this list **does** close when wrap is off. Expiry copy stays.
...
| `SessionPolicy.ERC20SpendCaps` (A7) | Yes (`omitempty`) | **UI-blocking.** Parallel with A0. Singular `ERC20SpendCap` remains alias |
...
| Second Send ERC-20 grant / catalog row to cap WETH | **Rejected.** Singleton replace. Cap another ERC-20 via **A7 `erc20SpendCaps[]`**, not a second policy. Native ETH is not that array. |
| Put ETH in `erc20SpendCap` | **Rejected.** `NativeTokenLimitModule` / Track A. |
...
| Q2 | Uniswap does not need native-send. Two `nativeTransfer` builders → **union of recipients, max of caps** (not min, not sum). Max **5** recipients. | K3 |
| Q3 | Max native recipients = **5**. A0 20-row **install** passed; 20-row deferred **replace** AA23'd at every seed tried. Cut because replace is the singleton re-grant path — not because install AA26'd. | K3, K14 |
...
| **High** | 20-row deferred replace AA23s (install at 1.5M passed) | K14: cut max to 5; seed +45k/row past 3, wired from packed install |
...
- **Files/components:** `api/openapi.yaml`; `make` oapi-codegen outputs under `aggregator/rest/generated/`; `model/session_policy.go`; `core/taskengine/session_permissions.go` (Validate + `HooksFor` packing; A2 packers already on staging); `aggregator/rest/handlers_policies.go` mapping; `aggregator/rest/handlers_policies_test.go`; `core/taskengine/engine_session_policy_test.go` (`TestSessionPermissionsValidation`).
- **Dependencies:** **A7** (rebase `permissionsFromAPI` on the cap slice + pointer). **A2** packing (landed first).
- **Description:** Additive `nativeRecipients` / `nativeSpendCap` / `allowContractRecipient`. Relax OpenAPI `required` (**JSON-additive; breaking generated Go** — nil-guard pointer **and** `erc20SpendCaps[]`). **Omit vs `[]`:** present empty arrays are 400. Validate rules (K2/K3/K4) including `eth_getCode==0`. **Refuse `len(nativeRecipients) > MaxNativeRecipients` (5).** Do not ship 20: deferred replace AA23s. `attachDeclaredPermissions` copies new fields. ERC-20-only `HooksFor` stays the 3-hook layout (NT hooks only when `nativeSpendCap` is set). `make storage-check` green.
...
- **Description:** Implement K1 hook order and K5 (AllowlistExec always). ERC-20-only `HooksFor` byte-identical to pre-A2 (golden). Native-only and mixed pack NT hooks. Promote spike packers into `aa`. **VGL seed scales with packed allowlist rows** (`AllowlistRows` set in `NewSessionResolver` next to `DeferredTeardownCount`; +45k/row past 3). 20-row **install** passed at 1.5M; 20-row **replace** AA23s — product max is 5, recorded as `MaxNativeRecipients` for A1. **One uninstall splitter; no 3-hook flat-reverse compatibility path.** **Do not merge until** the 5-hook uninstall unit test is green **and** A0 reads `limits(entity,account)==0` after replace. `TestHooksForAlwaysScopesSelectors` replaced by the split tests.
...
#### PR A7 — `feat: per-token ERC-20 spend caps on the session grant`
...
**Studio cannot adopt the shared-grant amount-list design until A7 is on `staging`.** Native send (A0–A5) is a separate train. **PR A6 is cancelled.** Track B production fund movement is **only B5 with the flag on**.
